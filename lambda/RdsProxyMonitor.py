...
def is_rds_proxy_target_available():
    ...
    print('Attempting to Retrieve Proxy Target Status for Proxy: "' + os.environ['APP_DB_PROXY_NAME'] + '"')
    
    try:
        
        describe_proxy_targets_resp = rds_client.describe_db_proxy_targets(
            DBProxyName = os.environ['APP_DB_PROXY_NAME'], 
            TargetGroupName = 'default'
        )
        
        print('Successfully Retrieved Proxy Target Status for Proxy: "' + os.environ['APP_DB_PROXY_NAME'] + '"')
...
def point_global_app_db_endpoints_to_failover_proxy():
    ...
    for endpoint_type in ['READER', 'WRITER']:
        ...
        custom_functions.update_dns_record(
            ...
            new_value       = os.environ['APP_DB_PROXY_' + endpoint_type + '_ENDPOINT'],
